...
## 6. 实现迭代与思考方式
- 迭代目标：先交付“可用的最小闭环”（CRUD + 列表展示），再逐步补齐需求点（分类/排序/时间/提醒），每一步都保证可回归（测试可跑、脚本可执行）
- 分层选择：保持 `api`（Controller/DTO）→ `service`（业务规则）→ `storage`（JdbcTemplate/文件）边界清晰，避免前端/存储细节污染业务逻辑
- 数据与接口策略：将 `dueDate` 统一为字符串在 API 层传递，但在服务层做格式归一化与校验，既降低前端对时区/序列化的依赖，也保证排序可用
- 可运维性：建表脚本尽量可重复执行（幂等创建/修正字段），减少“本地已经有旧表结构”导致启动失败的概率

### 6.1 关键迭代记录（按需求推进）
- 分类：新增 `category` 字段 + 分类过滤 + 分类列表接口，满足“先能用再优化体验”的路径
- 排序：支持创建时间/优先级/截止时间排序；排序逻辑收敛在服务层，存储层只负责读写
- 截止时间：由“日期”升级为“具体时间”，前端使用 `datetime-local`，后端统一归一化为 `yyyy-MM-ddTHH:mm`，数据库字段使用 `DATETIME`
- 必填规则：将 `dueDate` 从“可选”改为“必填”，通过前端 `required` + 后端 `@NotBlank` 双保险防止脏数据进入系统
- 提醒/通知：先实现浏览器侧的“页面在线提醒”（截止前 10 分钟），以最低成本覆盖核心场景；不引入服务端定时任务与外部推送依赖
